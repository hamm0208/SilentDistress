...
int FightScene(Player& pPlayer, Monster& pMonster) {
	...
	List<Decision> DecisionInFighting;
	int choice = 0;
...
void InitScene1(Scene& pCrashSiteScene, Player* pPlayer, Monster* pMonster, Item* AllItems[ALL_ITEM_COUNT]) {
	pCrashSiteScene.AddLoot(AllItems[0]);
	pCrashSiteScene.AddLoot(AllItems[0]);
	pCrashSiteScene.AddLoot(AllItems[0]);
	pCrashSiteScene.AddLoot(AllItems[0]);
	pCrashSiteScene.AddLoot(AllItems[2]);
	pCrashSiteScene.AddLoot(AllItems[2]);
	pCrashSiteScene.AddLoot(AllItems[2]);
	pCrashSiteScene.AddLoot(AllItems[5]);
	pCrashSiteScene.AddLoot(AllItems[9]);
	pCrashSiteScene.AddLoot(AllItems[7]);
	//Events for scene1
	/*
...
void InitScene2(Scene& pJungleClearingScene, Player* pPlayer, Monster* pMonster, Item* AllItems[ALL_ITEM_COUNT], Game& pGame) {
	pJungleClearingScene.AddLoot(AllItems[12]); // Bamboo Water
	pJungleClearingScene.AddLoot(AllItems[12]); // Bamboo Water
	pJungleClearingScene.AddLoot(AllItems[3]);  // Wild Berries
	pJungleClearingScene.AddLoot(AllItems[3]);  // Wild Berries
	pJungleClearingScene.AddLoot(AllItems[4]);  // Bananas
	pJungleClearingScene.AddLoot(AllItems[4]);  // Bananas
	pJungleClearingScene.AddLoot(AllItems[11]); // Jungle Roots
	pJungleClearingScene.AddLoot(AllItems[10]); // Dull Machete
	pJungleClearingScene.AddLoot(AllItems[8]);  // Wild Mint
	pJungleClearingScene.AddLoot(AllItems[8]);  // Wild Mint
...
int main() {
	...
	newGame.AttachRightScene(AllScenes[5]);						//Attach Hunting Grounds to Right Child of Deep Jungle
	newGame.Left();
	...
	for (Iterator1D<Scene*> iter(AllScenes, 7, 0); iter != iter.end(); ++iter) {
		if (*iter != nullptr) {
			delete* iter;
		}
	}
